...
from app.twin.trends import compute_trend, is_sensor_silent
...
def test_is_sensor_silent_accepts_naive_database_timestamp():
    db = MagicMock()
    latest = MagicMock()
    latest.timestamp = datetime.now(timezone.utc).replace(tzinfo=None) - timedelta(seconds=10)
    db.query().filter_by().order_by().first.return_value = latest

    assert is_sensor_silent(db, asset_id=1, sensor_type="vibration") is True
